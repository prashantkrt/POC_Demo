...
public class AsposePsdGenerator {
    ...
    public void downloadAndCreatePsd(String imageUrl, String logoUrl, String headerText,
                                     String fontName, int fontSize, String outputPath) throws Exception {

        File bgImageFile = downloadFile(imageUrl, ".jpg");
        File logoFile = downloadFile(logoUrl, ".png");

        RasterImage bgImage = (RasterImage) Image.load(bgImageFile.getAbsolutePath());
        RasterImage logo = (RasterImage) Image.load(logoFile.getAbsolutePath());
